...
class CustomerController extends BaseController
{
    ...
    {
        $loyalLevelId = $this->businessSettingService->findOneBy(criteria: ['key_name' => 'loyal_customer_tag', 'settings_type' => CUSTOMER_SETTINGS])->value['id'] ?? null;
        $loyalCustomer = 0;
        if ($loyalLevelId != null) {
            $loyalCustomer = $this->customerService->loyalCustomerCount($loyalLevelId);
        }
        $registered = $this->customerService->getBy(criteria: ['user_type' => CUSTOMER])->count();
        ...
        $verified = $this->customerService->getBy(criteria: ['user_type' => CUSTOMER, ['first_name', '!=', null], ['last_name', '!=', null]])->count();
        $new = $this->customerService->getBy(criteria: ['user_type' => CUSTOMER], orderBy: ['created_at' => 'asc'], limit: 10)->count();
        return response()->json(view(
            'usermanagement::admin.customer._statistics',
            compact('registered', 'inactive', 'new', 'verified', 'loyalCustomer', 'active')
        )->render());
    }
