...
class DashboardController extends BaseController
{
    ...
    public function index(?Request $request, string $type = null): View|Collection|LengthAwarePaginator|null|callable|RedirectResponse
    {
        ...
        $card = [
            'account' => $this->userAccountService->findOneBy(criteria: ['user_id' => $superAdmin?->id]),
    ...
    public function zoneWiseStatistics(Request $request)
    {
        ...
        $count = $this->tripRequestService->getAll()->count();
        return response()
            ->json(view('adminmodule::partials.dashboard._areawise-statistics', ['trips' => $data, 'totalCount' => $count])
                ->render());
    }
}
