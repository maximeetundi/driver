...
@section('content')
    ...
    <div class="main-content">
        <div class="container-fluid">
            ...
            {{--                    <h2 class="fs-22 text-capitalize">{{translate('discount_overview')}}</h2>--}}
            {{--                    <div>--}}
                                    ...
                                    @forelse ($discounts as $key => $discount)
                                        <tr id="hide-row-{{$discount->id}}" class="record-row">
                                            ...
                                            <td class="discount_status">
                                                ...
                                                @if(!$date->gt($discount->end_date) && !$discount->is_active)
                                                    <span
                                                        class="badge badge-danger">{{ translate('currently_off') }}</span>
                                                @elseif ($date->lt($discount->start_date))
                                                    <span
                                                        class="badge badge-warning">{{ translate('upcoming') }}</span>
                                                @elseif ($date->gt($discount->end_date))
                                                    <span
                                                        class="badge badge-success">{{ translate('completed') }}</span>
                                                @elseif ($date->lte($discount->end_date))
                                                    <span
                                                        class="badge badge-info">{{ translate(ONGOING) }}</span>
                                                @endif
                                            </td>
                                            @can('promotion_edit')
                                                <td class="status">
